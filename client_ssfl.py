...
from model import SimSiam, ResNet18, train_heterossfl, test, adjust_learning_rate, train_fedsimsiam
import numpy as np
import os
import time
import pandas as pd
...
class FedSimSiamClient(fl.client.NumPyClient):
    """Define a Flower FedSimSiam Client."""
    ...
    def __init__(self, cid, trainloader, valloader, radloader, num_classes, epochs, config: DictConfig, save_dir) -> None:
        ...
        self.radloader = radloader
        # a model that is randomly initialised at first
        self.model = SimSiam(backbone=ResNet18(num_classes, pretrained=False).resnet)
        # SimSiam(backbone=ResNet18(num_classes, pretrained=False).resnet)
        self.len_params = len(self.model.state_dict())
        self.phi_K_mean = None
        # figure out if this client has access to GPU support or not
    ...
    def fit(self, parameters, config: Dict[str, Scalar]):
        ...
        # print(f"Client {self.cid} training on {self.device}")
        # local training
        train_loss = train_fedsimsiam(self.model, self.trainloader, optim, epochs, self.device, hide_progress=True)
        # Possible artificial knowledge for testing: knn_monitor, local memory labeled dataset -> weighted_accuracies strategy side
        # return updated model params, number of examples and dict of metrics
        return self.get_parameters({}), len(self.trainloader.dataset), {"loss": train_loss, "client_id" : self.cid, "fit_mins": (time.time()-start)/60}
...
def generate_client_fedsimsiam_fn(trainloaders, valloaders, radloader, num_classes, epochs, exp_config, save_dir):
    """Return a function that can be used by the VirtualClientEngine.

    to spawn a FedSimSiam FlowerClient with client id `cid`.
    """
